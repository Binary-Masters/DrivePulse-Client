	...
	return (
		<div className="flex items-center text-xl text-white gap-2">
			{folderHistoryArr.map(
				({ currentPath, currentDirectory, subDirectoryLevel }) => (
					<span
						onClick={() => handleClick(currentPath)}
						key={subDirectoryLevel}
					>
						<span className="flex items-center justify-center gap-2">
							<span className="cursor-pointer hover:underline text-white/90">
								{currentDirectory}
							</span>
							<span className="flex pt-0.5 text-base text-white/40">
