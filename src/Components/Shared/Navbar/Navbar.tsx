...
export default function Navbar() {
  ...
  return (
    <div
      className={`px-2 navbar md:px-14 py-2 fixed top-0  z-50 ${
        sroll
          ? "glass text-white shadow-md border-b py-4 transition-all duration-300"
          : "text-white"
      }`}>
      <div className="text-2xl navbar-start gap-2 md:text-3xl">
        ...
        <div className="flex items-center gap-1">
          <Image
            src={logo}
            className="w-[30px] h-[30px] md:w-[45px] md:h-[50px]"
            alt="DrivePulse Logo"
          />
          <div className="">
            <h2
            ...
            <p
              ...
              className="text-[14px] md:text-[18px] font-medium md:-mt-2 -mt-3">
              PULSE
            </p>
          </div>
        </div>
        ...
        <ul className="w-full">
          {routes.map((route, i) => (
            <li key={route.path + route.label}>
              ...
              {routes.length - 1 !== i && <div className="my-2 divider"></div>}
            </li>
          ))}
        </ul>
      </div>
